<!--
✨ hanania16/hanania16 GitHub Profile README ✨
-->

<h1 align="center">Hi 👋, I'm Hanania Meseret</h1>
<h3 align="center">Software Engineering Student | Cloud, DevOps & AI-Focused Engineer</h3>

<!-- <p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Pacifico&size=32&duration=4000&pause=1000&color=6FA8DC&center=true&vCenter=true&width=700&height=70&lines=Cloud+%26+DevOps+Enthusiast+☁️⚙️;AI%2FML+Explorer+🤖;Building+Web+%26+App+Solutions+🚀;Design.+Code.+Deploy.+Scale." />
</p> -->

<!-- <p align="center">
  <img src="https://media.giphy.com/media/l0MYt5jPR6QX5pnqM/giphy.gif" width="280" />
</p> -->

---

## 👋 About Me

I’m a **software engineering student** passionate about **cloud infrastructure (Claude & modern AI platforms)**,  
**DevOps**, **AI/ML**, and **full-stack web & app development**.

I focus on building **real, usable systems** — not just writing code, but **designing, deploying, and improving products** that solve real problems.

What I bring:
- Systems thinking, not just features
- Clean, maintainable, scalable code
- Strong collaboration & clear communication
- A balance of **engineering + design**

---

## 🎯 What I Can Do

- 🌐 Build modern, responsive **web & app solutions**
- ⚙️ Design and implement **backend APIs**
- ☁️ Apply **DevOps & Linux fundamentals** for deployment-ready systems
- 🤖 Integrate **AI / ML-powered features**
- 🎨 Create clean **UI/UX designs using Figma**

---

## 🛠 Tech Stack

<div align="center">
  <table>
    <tr>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="48"/><br/>Python
      </td>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" width="48"/><br/>Java
      </td>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="48"/><br/>React
      </td>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nestjs/nestjs-original.svg" width="48"/><br/>NestJS
      </td>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="48"/><br/>MySQL
      </td>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="48"/><br/>Linux
      </td>
      <td align="center" width="96">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" width="48"/><br/>Figma
      </td>
    </tr>
  </table>
</div>

---

## 💼 Featured Project

<div align="center">
  <a href="https://github.com/hanania16/Pharmalook">
    <img src="https://media.giphy.com/media/3o7aCVP1G5wrPau8sc/giphy.gif" width="260"/>
  </a>

  <p>
    <strong>Pharmalook</strong><br/>
    A smart application that helps users locate nearby pharmacies and search for medicines efficiently.<br/>
    <em>Built with real-world usability, performance, and clarity in mind.</em>
  </p>
</div>

---

## 🚀 How I Work

- Problem-first development mindset
- Clean, readable, maintainable code
- Practical DevOps thinking (build → test → deploy)
- Focus on **real users and real needs**

---

## 📫 Connect With Me



## 🌐 Socials:
[![Discord](https://img.shields.io/badge/Discord-%237289DA.svg?logo=discord&logoColor=white)](https://discord.gg/hanania_16) [![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/hanania_16) [![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:hanania.meseret@gmail.com) 



![](https://github-readme-stats.vercel.app/api?username=hanania16&theme=one_dark_pro&hide_border=true&include_all_commits=true&count_private=true)<br/>
![](https://nirzak-streak-stats.vercel.app/?user=hanania16&theme=one_dark_pro&hide_border=true)<br/>
![](https://github-readme-stats.vercel.app/api/top-langs/?username=hanania16&theme=one_dark_pro&hide_border=true&include_all_commits=true&count_private=true&layout=compact)

## 🏆 GitHub Trophies
![](https://github-profile-trophy.vercel.app/?username=hanania16&theme=radical&no-frame=false&no-bg=false&margin-w=4)

### ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)

### 🔝 Top Contributed Repo
![](https://github-contributor-stats.vercel.app/api?username=hanania16&limit=5&theme=default&combine_all_yearly_contributions=true)

---
[![](https://visitcount.itsvg.in/api?id=hanania16&icon=0&color=0)](https://visitcount.itsvg.in)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
